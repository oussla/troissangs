package com.thequest.epist.argu {
	 ...
	 * @author nlgd
	 */
	public class MEProferation extends MediaElement {
		//--------------------------------------------------
		//
		//		Properties
		//
		...
		//
		//--------------------------------------------------
		public function MEProferation(p_ed:EDMedia) {
			super(p_ed);
		...
		override public function init() {
			...
			button_mc.label_tf.text = _translator.translate("proferation_btn_label").toUpperCase();
		}
		/**
		 * Chargement 
		 */
		public function loadImage() {
			//_trace.debug("MEProferation.loadImage : " + _ed.url);
			if (!_imgIsLoaded) {
		...
		override public function clear() {
			//_trace.debug("MEProferation.clear!");
			super.clear();
			if(_image_bm) {
		...
		private function _resize(width:int, height:int):void {
			//trace("MEProferation._resize : "+width+"x"+height);
		...
		private function _onClick(evt:MouseEvent):void {
			...
			_trace.debug("ClickProferation. OS:" + os + ", language:" + _translator.language + ", path:" + mdm.Application.path);
			AppNav.getInstance().setWindowed();
			var langSuffix:String = "";
			
			if (_translator.language == "en") langSuffix = "_us";
			if (os == "mac") {
				mdm.System.exec(mdm.Application.path + "proferation/proferation" + langSuffix + ".app");
			} else {
				mdm.System.exec(mdm.Application.path + "proferation/proferation" + langSuffix + ".exe");
			}
